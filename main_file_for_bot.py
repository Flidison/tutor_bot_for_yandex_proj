...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    telegram_id, name, role, confirmed, user_name = user
    if not confirmed:
    ...
    if confirmed:
        if role == "tutor":
            await message.reply_text("Привет спасибо большое, что пользуетесь ботом")
        else:
...
async def add_student_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Введите Username ученика:")
...
async def handle_student_registration(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "awaiting_id":
        try:
            student_id = update.message.text[1:]
            print(student_id)
            print(1)
        except ValueError:
        ...
        tgf=0
        print(student_id)
        user = await get_user(tgf,student_id)
        if not user:
